$(document).ready(function() {
    ...
    $("#whitelabel_frontofficebundle_demande__demande_auditEnergie_structure_id").on('change', function () {
        searchConseiller();
    });

    function addCriteriaEvents() {
        $("#whitelabel_frontofficebundle_demande__demande_auditEnergie_nbPersFoyer, #whitelabel_frontofficebundle_demande__demande_auditEnergie_revenu1, #whitelabel_frontofficebundle_demande__demande_auditEnergie_revenu2").on("change blur keyup load", function () {
            searchStructure();
        });
    }

    function searchStructure() {
        var structureId = parseInt($('#whitelabel_frontofficebundle_demande__demande_auditEnergie_structure_id').find(':selected').val(), 10);
        if (!structureId) {
            structureId = 0;
        }

        var nombrePersonneFoyer_ = $('#whitelabel_frontofficebundle_demande__demande_auditEnergie_nbPersFoyer').val();
        var revenuFiscalRef_ = $('#whitelabel_frontofficebundle_demande__demande_auditEnergie_revenu3').val();
        ...
        if (
            ...
            $.ajax({
                ...
                $('#whitelabel_frontofficebundle_demande__demande_auditEnergie_structure_id').empty();
                $('#whitelabel_frontofficebundle_demande__demande_auditEnergie_structure_id').append('<option value="" selected>-- Choisir une structure --</option>');

                /* liste des structures Recommandées */
                if (!$.isEmptyObject(data.advisedStructureList)) {
                    $('#whitelabel_frontofficebundle_demande__demande_auditEnergie_structure_id').append('<optgroup label="Recommandées">');
                    $.each(data.advisedStructureList, function (key, value) {
                        var selected = '';
                        if (structureId == key) {
                            selected = ' selected="selected"';
                        }
                        $('#whitelabel_frontofficebundle_demande__demande_auditEnergie_structure_id').append(
                            '<option value="' + key + '" ' + selected + '>' + value + '</option>'
                        );
                    });
                    $('#whitelabel_frontofficebundle_demande__demande_auditEnergie_structure_id').append('</optgroup>');
                }

                /* liste des structures Autres */
                if (!$.isEmptyObject(data.otherStructureList)) {
                    $('#whitelabel_frontofficebundle_demande__demande_auditEnergie_structure_id').append('<optgroup label="Autres">');
    ...
    function searchConseiller() {
        var structureId_ = $('#whitelabel_frontofficebundle_demande__demande_auditEnergie_structure_id').find(':selected').val();
